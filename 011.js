...
var gright = 0,
	...
	gdiag = 0,
	gdiag_rev = 0,
	ingrid = grid[0].length,
	outgrid = grid.length,
	prod = 0;

for (var i = 0; i < outgrid; i++) {
	for (var j = 0; j < ingrid - 4; j++) {
...
for (var i = 0; i < ingrid; i++) {
	for (var j = 0; j < outgrid - 4; j++) {
...
grid.forEach(function(item) {
	new_grid.push(item.reverse());
});

for (var i = 0; i < outgrid - 4; i++) {
	for (var j = 0; j < ingrid - 4; j++) {
		prod = new_grid[i][j] * new_grid[i + 1][j + 1] * new_grid[i + 2][j + 2] * new_grid[i + 3][j + 3];
		if (prod > gdiag_rev) {
			gdiag_rev = prod;
		}
	}
